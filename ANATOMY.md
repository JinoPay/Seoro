...
4. **서비스 등록** (`:63-91`): **29개 전부 Singleton**
...
| 파일 | 줄수 | 역할 |
|------|------|------|
| `Shared/Components/Sidebar/SessionList.razor` | ~634 | 세션 목록 (Phase 3에서 719→634줄 감소) |
...
### 빠진 것 / 문제점
- ~~**SessionList 719줄 God Component**~~ → ✅ **부분 해결**: Phase 3에서 `SessionItem.razor`(세션 행)과 `SessionListToolbar.razor`(툴바+필터)를 자식 컴포넌트로 추출. 719→634줄로 12% 감소
...
- **테스트 0개**: 전체 코드베이스에 단위/통합 테스트 없음. 다만 Phase 3에서 추출된 서비스(SystemPromptBuilder, SessionInitializer, ChatPrWorkflowService)는 독립 테스트 가능한 구조
...
| **7** | SessionList God Component (719줄) | 사이드바 변경이 복잡 | `SessionList.razor` | ✅ **부분 해결** — Phase 3: `SessionItem`+`SessionListToolbar` 자식 컴포넌트 추출. 719→634줄(12%↓) |
