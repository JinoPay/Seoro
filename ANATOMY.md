...
- ~~**ModelDefinitions 하드코딩**: opus/sonnet/haiku 3개 모델만. 새 모델 추가는 코드 변경 필요~~ → ✅ **해결**: `ModelDefinitions`가 `models.json` 외부 설정 파일 로딩 지원. 가격 정보(`ModelPricing`) 통합. 키워드 기반 정규화. 기본 내장값 폴백
...
| 파일 | 줄수 | 역할 |
|------|------|------|
| `Shared/Services/GitService.cs` | ~444 | 모든 git 명령 (`IProcessRunner` 기반) |
...
| **6** | 프로세스 실행 4곳 복붙 | 타임아웃/에러처리 불일치 | Git/Gh/Claude/Hooks | ✅ **해결** — 7개 서비스 `IProcessRunner` 마이그레이션 완료. `CominomiConstants.Env` 환경변수 통합. ClaudeService/CloneAsync만 스트리밍 특성상 직접 Process 유지 |
...
| **9** | 가격/모델/CLI플래그 하드코딩 | 외부 변경마다 코드 수정 필요 | `UsageService.cs`, `ModelDefinitions.cs`, `ClaudeArgumentBuilder.cs` | ✅ **해결** — `ModelDefinitions`에 `models.json` 로딩 + 가격 통합. 요약 모델/프롬프트/타임아웃 → `AppSettings`. 중복 문자열 → `CominomiConstants` |
