...
**코드 규모**: ~164개 소스 파일, ~16,400줄 (Cominomi.Shared에 집중, tests/Cominomi.Shared.Tests 포함)
...
| 새 파일 / 변경 내용 | 역할 / 영향 범위 |
|---------------------|-----------------|
| `tests/Cominomi.Shared.Tests/` (프로젝트 신규) | xUnit 테스트 인프라 — 84개 테스트 (ContentGrouper 14, QuestionDetector 11, ToolDisplayHelper 13, ExtractToolResultContent 6, SessionStatusMachine 35, +α) |
...
- ~~**테스트 0개**: 전체 코드베이스에 단위/통합 테스트 없음~~ → ✅ **해결**: Phase 4에서 `tests/Cominomi.Shared.Tests/` 프로젝트 도입. 84개 테스트 (ContentGrouper 14, QuestionDetector 11, ToolDisplayHelper 13, ExtractToolResultContent 6, SessionStatusMachine 35). `InternalsVisibleTo`로 internal 메서드 테스트 가능
...
| 순위 | 문제 | 영향 | 관련 파일 | 상태 |
|------|------|------|-----------|------|
| **1** | ChatView God Component | 모든 기능 변경이 하나의 파일에 집중 | `ChatView.razor` | ✅ **해결** — Phase 1: `StreamEventProcessor` 추출, Phase 3: `SystemPromptBuilder`+`SessionInitializer`+`ChatPrWorkflowService` 추출. 1,461→899줄(38%↓), 18→14개 서비스 |
| **2** | JSON 영속화에 파일 락 없음 | 병렬 세션의 동시 저장 시 데이터 손상 | `SessionService.cs` | ✅ **해결** — `AtomicFileWriter` + per-session `SemaphoreSlim` 락 |
| **3** | ~~세션 파일에 전체 메시지 저장~~ | ~~무한 성장, 로드 성능 저하~~ | `Session.cs`, `SessionService.cs` | ✅ **해결** — Phase 4: 메타데이터/메시지 분리 저장 + 도구 출력 2,000자 절단. 이전 형식 자동 호환 |
| **4** | ChatState God Object (30+ 멤버) | 5가지 관심사 혼재, 테스트 불가 | `ChatState.cs` | ✅ **해결** — Phase 3: 파사드 패턴 + `MessageManager`/`StreamingStateManager`/`SettingsStateManager` 분리. 366→219줄(40%↓) |
| **5** | ~~모델 검증 완전 부재~~ | ~~잘못된 상태 전이~~ | `Session.cs`, `SessionStatusMachine.cs` | ✅ **부분 해결** — Phase 4: `SessionStatusMachine` + `Session.TransitionStatus()` + `private set`. 기타 모델 검증은 미해결 |
| **6** | ~~프로세스 실행 복붙~~ | ~~타임아웃/에러처리 불일치~~ | | ✅ **해결** — 7개 서비스 `IProcessRunner` 마이그레이션 완료 |
| **7** | ~~SessionList God Component (719줄)~~ | ~~사이드바 변경이 복잡~~ | `SessionList.razor` | ✅ **해결** — Phase 3: `SessionItem`+`SessionListToolbar` 추출(719→634줄). Phase 4: `SessionListDataService` 추출(634→439줄). 총 39%↓ |
| **8** | PR 생성 경로 2개 (AI vs 직접) | 결과 불일치 가능 | `ChatView.razor`, `SessionGitWorkflowService.cs` | ✅ **해결** — Phase 3: `IChatPrWorkflowService`로 PR 워크플로우 통합 |
| **9** | ~~가격/모델/CLI플래그 하드코딩~~ | ~~외부 변경마다 코드 수정 필요~~ | | ✅ **해결** — `ModelDefinitions` + `models.json` + `CominomiConstants` |
| **10** | ~~테스트 0개~~ | ~~리팩터링 위험, 회귀 감지 불가~~ | 전체 | ✅ **해결** — Phase 4: `tests/Cominomi.Shared.Tests/` 도입. 84개 테스트 (순수 로직 + 상태 머신) |
| **신규** | `async void` 안티패턴 (6개 컴포넌트) | 예외 미관찰, UI 무반응 가능 | ChatView, DetailPanel, MainLayout, SessionList, SidebarChanges, SidebarExplorer | ✅ **해결** — Phase 3: `void` + `InvokeAsync()` 안전 패턴 |
| **신규** | ~~Spotlight 크래시 시 리포 상태 미복구~~ | ~~데이터 손실 위험~~ | `SpotlightService.cs` | ✅ **해결** — Phase 4: `spotlight-state.json` 영속화 + `RecoverAsync()` + 동시 Spotlight 가드 + sessionId별 stash |
