    ...
    ILogger<PullRequestService> logger) : IPullRequestService
{
    ...
    public async Task<TrackedPullRequest?> RefreshAsync(Session session, CancellationToken ct = default)
    {
        ...
        if (current == null || string.IsNullOrWhiteSpace(current.Url) || string.IsNullOrWhiteSpace(session.Git.WorktreePath))
            return null;
    ...
    private async Task<TrackedPullRequest?> QueryAsync(string workingDir, string urlOrNumber, CancellationToken ct)
    {
        var ghPath = await ResolveGhPathAsync();
        var result = await processRunner.RunAsync(new ProcessRunOptions
        {
            ...
            [
                "pr", "view", urlOrNumber,
                "--json",
                "number,url,state,isDraft,mergeable,mergeStateStatus,reviewDecision,headRefName,baseRefName,mergedAt,mergeCommit,statusCheckRollup"
            ],
        ...
        if (!result.Success)
        {
            logger.LogDebug("gh pr view 실패: {Err}", string.IsNullOrWhiteSpace(result.Stderr) ? result.Stdout : result.Stderr);
            return null;
        }
        ...
        try
        {
            ...
            {
                Url = root.TryGetProperty("url", out var urlEl) ? urlEl.GetString() ?? urlOrNumber : urlOrNumber,
